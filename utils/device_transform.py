...
def _to_grayscale(x: Tensor) -> Tensor:
    """Convert RGB tensor to grayscale by computing BT.601 luminance.

    Accepts any shape with a channels-dim of size 3 at ``dim=-3`` (e.g.
    ``[B, T, 3, H, W]`` or ``[N, 3, H, W]``).  Returns the same shape with
    the luminance value copied across all three channels.
    """
    # Coefficients: 0.299 R + 0.587 G + 0.114 B  (BT.601 / NTSC luma)
    r = x[..., 0:1, :, :]
    g = x[..., 1:2, :, :]
    b = x[..., 2:3, :, :]
    luma = 0.299 * r + 0.587 * g + 0.114 * b
    return luma.expand_as(x).contiguous()
...
class DeviceMattingTransform(nn.Module):
    ...
    def forward(self, batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # ---- Grayscale augmentation (input + FG target) ----
        # With some probability, strip all colour from both the model input
        # and the clean foreground supervision target.  This forces the
        # network to solve matting from structure / edges / alpha cues alone
        # and dramatically improves robustness to green-spill artifacts and
        # misleading background colours.  BG is also converted so the
        # composite stays self-consistent.
        if cfg.grayscale_augment_p > 0.0 and random.random() < cfg.grayscale_augment_p:
            input_gt = _to_grayscale(input_gt)
            fg = _to_grayscale(fg)
            bg = _to_grayscale(bg)
            if emit_global_context and global_input_gt is not None:
                global_input_gt = _to_grayscale(global_input_gt)
...
def build_device_transform_from_data_cfg(
    ...
    cfg = DeviceMattingTransformConfig(
        ...
        grayscale_augment_p=float(data_cfg.get("grayscale_augment_p", 0.0)),
    )
